...
- 하단 중앙: 현재 발화자 이름 + 대사 자막 (이전 대사는 즉시 사라짐, 카톡식 로그 없음)

레이아웃은 CSS Flexbox 세로 배치(`src/style.css`의 `.stage`)로 고정 높이 구역
(헤더 → primary 그리드 → secondary 그리드 → 자막 영역)을 쌓는 방식입니다.
...
- 활성화 시 얇은 테두리 + 은은한 그림자 + 4칸 오디오 미터 애니메이션이 켜지고,
  전환은 CSS `transition`으로 자연스럽게 처리됩니다(번쩍임/확대 없음).
- `role: "reaction"`(엄지)은 활성화 색상이 앰버색으로 구분되어 "짧은 반응"임이
  시각적으로 다르게 보입니다.
